...
// POST /bookings — create a booking (public — used by clients)
router.post('/', async (req, res) => {
  const { service_id, booked_at, client_notes, client } = req.body;
  // client: { name, phone, email }

  if (!service_id || !booked_at || !client?.name) {
    return res.status(400).json({
      error: 'service_id, booked_at and client.name are required',
    });
  }
  ...
  try {
    ...
    // 2. Check slot is still free (re-check at write time to prevent race conditions)
    ...
    const conflictResult = await dbClient.query(
      `SELECT b.id FROM bookings b
       ...
       WHERE b.status != 'cancelled'
         AND b.booked_at < $1
         AND (b.booked_at + (s.duration_mins || ' minutes')::interval) > $2`,
    ...
    // 3. Upsert client (match by phone or email if they exist)
    let clientId;
    if (client.phone || client.email) {
      ...
      if (existing.rowCount > 0) {
        ...
        // Update their name/contact in case it changed
    ...
    const bookingResult = await dbClient.query(
      `INSERT INTO bookings (client_id, service_id, booked_at, status, client_notes)
    ...
    // 5. Send confirmation emails (async — don't block the response)
    ...
    res.status(201).json({
      ...
      booking: {
        ...
        service: { name: service.name, duration_mins: service.duration_mins, price: service.price },
      },
    });
...
router.get('/', requireAuth, async (req, res) => {
  ...
  if (date) {
    params.push(date);
    query += ` AND b.booked_at::date = $${params.length}`;
  }
  if (status) {
    params.push(status);
...
// GET /bookings/:id — get one booking (admin)
...
// PATCH /bookings/:id/status — confirm or cancel (admin)
  ...
  try {
    ...
    if (status === 'cancelled') {
      const details = await db.query(
        `SELECT c.*, s.name AS service_name, s.duration_mins
         FROM bookings b
         JOIN clients c ON c.id = b.client_id
         JOIN services s ON s.id = b.service_id
         WHERE b.id = $1`,
      ...
      if (details.rowCount > 0) {
        ...
        sendCancellationEmail({
          client: row,
          service: { name: row.service_name, duration_mins: row.duration_mins },
          booking,
...
// POST /bookings/admin — create booking manually (admin, no availability check)
router.post('/admin', requireAuth, async (req, res) => {
  const { client_id, service_id, booked_at, client_notes, status } = req.body;

  if (!client_id || !service_id || !booked_at) {
    return res.status(400).json({ error: 'client_id, service_id and booked_at are required' });
  }
  ...
  try {
    const result = await db.query(
      `INSERT INTO bookings (client_id, service_id, booked_at, status, client_notes)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [client_id, service_id, booked_at, status || 'confirmed', client_notes || null]
    );
